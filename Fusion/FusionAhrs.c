 ...
 */
void FusionAhrsSetSettings(
	FusionAhrs* const ahrs,
	const FusionAhrsSettings* const settings) {
	ahrs->settings.gain = settings->gain;
	if ((settings->accelerationRejection == 0.0f)
		|| (settings->rejectionTimeout == 0)) {
		ahrs->settings.accelerationRejection = FLT_MAX;
	} else {
		ahrs->settings.accelerationRejection = powf(
			0.5f
				* sinf(FusionDegreesToRadians(settings->accelerationRejection)),
			2);
	}
	if ((settings->magneticRejection == 0.0f)
		|| (settings->rejectionTimeout == 0)) {
		ahrs->settings.magneticRejection = FLT_MAX;
	} else {
		ahrs->settings.magneticRejection = powf(
			0.5f * sinf(FusionDegreesToRadians(settings->magneticRejection)),
			2);
	}
...
#define Q ahrs->quaternion.element
	...
	FusionVector
		halfGravity;  // third column of transposed rotation matrix scaled by 0.5
	halfGravity.axis.x = Q.x * Q.z - Q.w * Q.y;
	halfGravity.axis.y = Q.y * Q.z + Q.w * Q.x;
	halfGravity.axis.z = Q.w * Q.w - 0.5f + Q.z * Q.z;
	...
	if (FusionVectorIsZero(accelerometer) == false) {
		...
		if (ahrs->accelerationRejectionTimer
		...
		ahrs->halfAccelerometerFeedback = FusionVectorCrossProduct(
			FusionVectorNormalise(accelerometer),
			halfGravity);
		...
		if ((ahrs->initialising == true)
			...
			ahrs->accelerationRejectionTimer -=
				ahrs->accelerationRejectionTimer >= 10 ? 10 : 0;
		} else {
			ahrs->accelerationRejectionTimer++;
		}
	}
	...
	if (FusionVectorIsZero(magnetometer) == false) {
		...
		if (ahrs->magneticRejectionTimer > ahrs->settings.rejectionTimeout) {
			FusionAhrsSetHeading(
				ahrs,
				FusionCompassCalculateHeading(halfGravity, magnetometer));
			ahrs->magneticRejectionTimer = 0;
		...
		FusionVector
			halfWest;  // second column of transposed rotation matrix scaled by 0.5
		halfWest.axis.x = Q.x * Q.y + Q.w * Q.z;
		halfWest.axis.y = Q.w * Q.w - 0.5f + Q.y * Q.y;
		halfWest.axis.z = Q.y * Q.z - Q.w * Q.x;

		// Calculate magnetometer feedback scaled by 0.5
		ahrs->halfMagnetometerFeedback = FusionVectorCrossProduct(
			FusionVectorNormalise(
				FusionVectorCrossProduct(halfGravity, magnetometer)),
			halfWest);
		...
		if ((ahrs->initialising == true)
			...
			ahrs->magneticRejectionTimer -=
				ahrs->magneticRejectionTimer >= 10 ? 10 : 0;
		} else {
			ahrs->magneticRejectionTimer++;
		}
	}
...
#define Q ahrs->quaternion.element
	...
	magnetometer.axis.x = cosf(headingRadians);
	magnetometer.axis.y = -1.0f * cosf(roll) * sinHeadingRadians;
	magnetometer.axis.z = sinHeadingRadians * sinf(roll);
...
#define Q ahrs->quaternion.element
	FusionVector gravity;  // third column of transposed rotation matrix
	gravity.axis.x = 2.0f * (Q.x * Q.z - Q.w * Q.y);
	gravity.axis.y = 2.0f * (Q.y * Q.z + Q.w * Q.x);
	gravity.axis.z = 2.0f * (Q.w * Q.w - 0.5f + Q.z * Q.z);

	const FusionVector linearAcceleration =
		FusionVectorSubtract(ahrs->accelerometer, gravity);
...
#define A ahrs->accelerometer.axis
	...
	FusionVector
		earthAcceleration;	// rotation matrix multiplied with the accelerometer, with 1 g subtracted
	earthAcceleration.axis.x = 2.0f
		* ((qwqw - 0.5f + Q.x * Q.x) * A.x + (qxqy - qwqz) * A.y
		   + (qxqz + qwqy) * A.z);
	earthAcceleration.axis.y = 2.0f
		* ((qxqy + qwqz) * A.x + (qwqw - 0.5f + Q.y * Q.y) * A.y
		   + (qyqz - qwqx) * A.z);
	earthAcceleration.axis.z = (2.0f
								* ((qxqz - qwqy) * A.x + (qyqz + qwqx) * A.y
...
FusionAhrsGetInternalStates(const FusionAhrs* const ahrs) {
	const FusionAhrsInternalStates internalStates = {
		.accelerationError = FusionRadiansToDegrees(FusionAsin(
			2.0f * FusionVectorMagnitude(ahrs->halfAccelerometerFeedback))),
		.accelerometerIgnored = ahrs->accelerometerIgnored,
		.accelerationRejectionTimer = ahrs->settings.rejectionTimeout == 0
			? 0.0f
			: (float)ahrs->accelerationRejectionTimer
				/ (float)ahrs->settings.rejectionTimeout,
		.magneticError = FusionRadiansToDegrees(FusionAsin(
			2.0f * FusionVectorMagnitude(ahrs->halfMagnetometerFeedback))),
		.magnetometerIgnored = ahrs->magnetometerIgnored,
		.magneticRejectionTimer = ahrs->settings.rejectionTimeout == 0
			? 0.0f
			: (float)ahrs->magneticRejectionTimer
				/ (float)ahrs->settings.rejectionTimeout,
	};
...
#define Q ahrs->quaternion.element
	const float yaw =
		atan2f(Q.w * Q.z + Q.x * Q.y, 0.5f - Q.y * Q.y - Q.z * Q.z);
	const float halfYawMinusHeading =
		0.5f * (yaw - FusionDegreesToRadians(heading));
	FusionQuaternion rotation;
	rotation.element.w = cosf(halfYawMinusHeading);
	rotation.element.x = 0.0f;
	rotation.element.y = 0.0f;
	rotation.element.z = -1.0f * sinf(halfYawMinusHeading);
